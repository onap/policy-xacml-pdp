...
/**
 * This class wraps a distributor so that it can be activated as a complete service together with all its xacml pdp
 * and forwarding handlers.
 */
public class XacmlPdpActivator {
    // The logger for this class
    private static final Logger LOGGER = FlexLogger.getLogger(XacmlPdpActivator.class);
    ...
    private void startXacmlPdpRestServer() throws PolicyXacmlPdpException {
        ...
        if (!restServer.start()) {
            throw new PolicyXacmlPdpException(
                    "Failed to start xacml pdp rest server. Check log for more details...");
        }
    }
