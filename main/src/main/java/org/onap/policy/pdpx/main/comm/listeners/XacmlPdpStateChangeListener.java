...
public class XacmlPdpStateChangeListener extends ScoListener<PdpStateChange> {
    ...
    public void onTopicEvent(CommInfrastructure infra, String topic, StandardCoderObject sco, PdpStateChange message) {
        ...
        try {
            ...
            // Begin sending periodic heartbeat status to the PAP if not started
            XacmlPdpHeartbeatPublisher heartbeat = new XacmlPdpHeartbeatPublisher(client, message.getState());

            // Terminate heartbeat if PDP State is Terminated
            if (PdpState.TERMINATED.equals(message.getState()) && XacmlPdpHeartbeatPublisher.isAlive()) {
                heartbeat.terminate();
                XacmlPdpActivator.getCurrent().stop();
            } else {
