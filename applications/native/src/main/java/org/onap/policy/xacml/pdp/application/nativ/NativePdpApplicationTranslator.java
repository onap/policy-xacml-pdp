...
public class NativePdpApplicationTranslator implements ToscaPolicyTranslator {
        ...
        try {
            ...
            for (int i = 0; i < keys.size(); i++) {
                ...
                if (keyObject instanceof Map<?, ?>) {
                    keyList = processKeyObject(keyObject, keyList, factory, datatype, apply);
                } else {
    ...
    private List<Object> processKeyObject(Object keyObject, List<Object> keyList,
                           ObjectFactory factory, String datatype, ApplyType apply)
        throws ToscaPolicyConversionException {
    ...
    private void parseTokens(int tokenType, StreamTokenizer tokenizer,
        ...
        try {
                ...
                if (tokenType == StreamTokenizer.TT_WORD) {
                    ...
                    if (token.toString().equals(DOUBLE) || token.toString().equals(INTEGER)) {
                        if (tokenizer.nextToken() == '(') {
                            ...
                            tokenizer.pushBack();
                            LOGGER.error("Keyword integer or double should be followed by (");
                            throw new ToscaPolicyConversionException(
                                "Keyword integer or double should be followed by ( ");
                        }
                    } else {
            ...
            LOGGER.error("Error parsing expr, could not get next token.");
            throw new ToscaPolicyConversionException("Error parsing expr, could not get next token.");
        }
    }
        ...
        try {
            ...
            if (Boolean.FALSE.equals(isWordOperator)) {
                ...
                if (value == '<' || value == '>' || value == '=' || value == '!') {
                    ...
                    if (Boolean.TRUE.equals(singleOperandExpression(operators.peek().toString()))) {
                        single = true;
                    }
                    while (Boolean.FALSE.equals(operators.isEmpty())
                    ...
                    if (Boolean.FALSE.equals(operators.isEmpty()) && Boolean.FALSE.equals(single)) {
            ...
            if (Boolean.FALSE.equals(isProcessed) && Boolean.TRUE.equals(isValidToken(token))) {
                while (Boolean.FALSE.equals(operators.isEmpty())
                    && Boolean.FALSE.equals(isPreviousOpLeftPar(operators))
    ...
    private Deque<Object> processOperator(Deque<Object> operators, Deque<Object> operands, ObjectFactory factory)
        throws ToscaPolicyConversionException {
        try {
            String op = "";
    ...
    private void processSingleOperandExpr(Deque<Object> operands, String op, ObjectFactory factory)
        ...
        try {
            ...
            LOGGER.error("Error while processing single operand expr");
            throw new ToscaPolicyConversionException("Error while processing single operand expr");
        }
    }
    ...
    private void processDoubleOperandExpr(Deque<Object> operands, String op, ObjectFactory factory)
        ...
        try {
            ...
            LOGGER.error("Error while processing double operand expr");
            throw new ToscaPolicyConversionException("Error while processing double operand expr");
        }
    }
