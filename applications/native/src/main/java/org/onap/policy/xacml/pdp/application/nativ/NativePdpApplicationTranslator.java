...
public class NativePdpApplicationTranslator implements ToscaPolicyTranslator {
    ...
    private TargetType setTargetType(Map<String, Object> appliesTo) throws ToscaPolicyConversionException {
        ...
        try {
            ...
            LOGGER.error("POLICY-500: Invalid target format");
            throw new ToscaPolicyConversionException("POLICY-500: Invalid target format");
        }
    ...
    private void parseTokens(int tokenType, StreamTokenizer tokenizer,
        ...
        try {
                ...
                if (tokenType == StreamTokenizer.TT_WORD) {
                    ...
                    if (token.toString().equals(DOUBLE) || token.toString().equals(INTEGER)) {
                        if (tokenizer.nextToken() == '(') {
                            ...
                            tokenizer.pushBack();
                            LOGGER.error("POLICY-500: Keyword integer or double should be followed by (");
                            throw new ToscaPolicyConversionException(
                                "POLICY-500: Keyword integer or double should be followed by ( ");
                        }
                    } else {
            ...
            LOGGER.error("POLICY-500: Error parsing expr, could not get next token.");
            throw new ToscaPolicyConversionException("POLICY-500: Error parsing expr, could not get next token.");
        }
    }
