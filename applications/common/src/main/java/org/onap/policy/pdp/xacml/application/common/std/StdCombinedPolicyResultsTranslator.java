...
public class StdCombinedPolicyResultsTranslator extends StdBaseTranslator {
    ...
    protected void scanContentObligation(Obligation obligation, DecisionResponse decisionResponse) {
        ...
        Map<String, Object> policyContent = null;
        for (AttributeAssignment assignment : obligation.getAttributeAssignments()) {
            ...
            if (pair == null) {
                LOGGER.warn("Attribute Found was null");
                continue;
            }
            LOGGER.info("Attribute Found: {}", pair.getLeft());
            //
            // Combined results need the policy id and the contents obligations
            //
            if (ToscaDictionary.ID_OBLIGATION_POLICY_ID.equals(pair.getLeft())) {
                policyId = pair.getRight().toString();
            } else if (ToscaDictionary.ID_OBLIGATION_POLICY_CONTENT.equals(pair.getLeft())) {
                policyContent = (Map<String, Object>) pair.getRight();
            } else {
                LOGGER.warn("Attribute not supported in this translator");
            }
        }
